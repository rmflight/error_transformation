# Visualizing Effect of Data Transformations on Errors
...
structure. In this work we demonstrate that a set of three simple graphs can 
facilitate the detection of proportional and mixed error in omics data. The
graphs examine the 1) absolute range, 2) standard deviation and 3) relative
standard deviation across replicates against the mean signal across replicates.
In addition to showing the presence of different types of error, they also
readily demonstrate the effect of various transformations on the error structure
as well. Using these graphical summaries find that the log-transform is the most
